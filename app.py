...
try:
    ...
    class _DummyScaler:
        """Pass-through scaler used when no persisted scaler file is available."""
        def transform(self, X):
            return X
        def inverse_transform(self, X):
            return X

    # Prefer an existing model file: support both legacy .h5 and new .keras formats
    LSTM_MODEL_PATH_H5 = "parking_lstm_model.h5"
    LSTM_MODEL_PATH_KERAS = "parking_lstm_model.keras"
    if os.path.exists(LSTM_MODEL_PATH_H5):
        LSTM_MODEL_PATH = LSTM_MODEL_PATH_H5
    elif os.path.exists(LSTM_MODEL_PATH_KERAS):
        LSTM_MODEL_PATH = LSTM_MODEL_PATH_KERAS
    else:
        raise FileNotFoundError("No LSTM model file found (.h5 or .keras)")
    ...
    # Try to load scaler; if missing, use pass-through scaler and continue
    if os.path.exists(LSTM_SCALER_PATH):
        with open(LSTM_SCALER_PATH, 'rb') as f:
            lstm_scaler = pickle.load(f)
        print("[LSTM] Scaler loaded successfully")
    else:
        lstm_scaler = _DummyScaler()
        print("[LSTM] Warning: scaler file not found; using identity scaler (predictions may be less accurate)")
    ...
    lstm_scaler = None
    LSTM_ENABLED = False
    print(f"[LSTM] Model not found. Train model first with: python train_lstm_model.py ({e})")
